  ...
  def init( opts, _args ) do
    ...
    graph = case !!opts[:seconds] do
      false ->
        ...
        |> line({{0,back_size}, {0, hour_size}}, pin: {0,0}, stroke: {thick, draw_color}, id: :hour_hand)
        |> line({{0,back_size}, {0, minute_size}}, pin: {0,0}, stroke: {thick, draw_color}, id: :minute_hand)
      true ->
        Graph.build(t: {-radius,radius})
        |> circle(radius, fill: background_color, stroke: {thick, draw_color})
        |> line({{0,back_size}, {0, hour_size}}, pin: {0,0}, stroke: {thick, draw_color}, id: :hour_hand)
        |> line({{0,back_size}, {0, minute_size}}, pin: {0,0}, stroke: {thick, draw_color}, id: :minute_hand)
        |> line({{0,back_size}, {0, second_size}}, pin: {0,0}, stroke: {thick, second_color}, id: :second_hand)
    end
